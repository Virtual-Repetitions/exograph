...
fn expand_dynamic_default_values(
    ...
    fn matches(
        ...
        match (field_type, context_type) {
            ...
            (FieldType::Plain(field_type), FieldType::Optional(context_type)) => match context_type.as_ref() {
                FieldType::Plain(inner_context_type) => field_type.name() == inner_context_type.name(),
                _ => false,
            },
            _ => false,
        }
