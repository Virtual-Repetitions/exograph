...
use crate::introspection_guard::IntrospectionGuardedResolver;
...
pub async fn create_system_router_from_system(
    ...
    let graphql_router = {
        ...
        // When the introspection gate is configured (the playground's
        // GitHub-OAuth gate and/or EXO_INTROSPECTION_SECRET), hold
        // introspection to it — gating only the playground HTML would leave
        // the schema publicly fetchable.
        let introspection_auth =
            IntrospectionAuthConfig::from_env(env.as_ref()).map_err(SystemLoadingError::Config)?;
        let introspection_resolver: Option<Arc<dyn SubsystemGraphQLResolver + Send + Sync>> =
            if introspection_auth.is_enforced() {
                introspection_resolver.map(|resolver| {
                    Arc::new(IntrospectionGuardedResolver::new(
                        resolver,
                        introspection_auth,
